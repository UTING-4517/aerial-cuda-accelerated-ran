...
function [Xtf, Xtf_noNoise] = ulRxBf(Xtf, Xtf_noNoise, pduList, srsState, puschTable, Chan_UL, SimCtrl)
    ...
    %   pduList - List of PUSCH PDUs
    %   srsState - SRS state
    %   puschTable - PUSCH configuration table
    %   Chan_UL - Channel information
    ...
    %     based on the PUSCH allocation parameters.
    ...
    % extract BWPSize and prgSize from first PUSCH PDU
    BWPSize = 273;
    prgSize = 2;
    for idxPdu = 1:nPdu
        if strcmp(pduList{idxPdu}.type, 'pusch')
    ...
    for idxPdu = 1:nPdu
        pdu = pduList{idxPdu};
        if strcmp(pdu.type, 'pusch')  % Ensure we're only processing PUSCH PDUs
    ...
    for idxUeg = 1:length(UegList)
        % Get PUSCH parameters for this UEG
        ...
        if SimCtrl.enableSrsState == 0
            ...
            for idxUe = 1:length(UegList{idxUeg}.idxPdu)
                ...
                H_ue = Chan_UL{pdu.idxUE+1}.chanMatrix.';
                H_ueg = [H_ueg, H_ue];
    ...
    Xtf = reshape(Xtf1, nRe, nSym, SimCtrl.CellConfigPorts);
