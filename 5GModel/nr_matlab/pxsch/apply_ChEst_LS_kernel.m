...
function [H_est, R0_acc, R1_acc] = apply_ChEst_LS_kernel(H_est, Y, r_dmrs, ChEst_par)
...
global_est_idx = global_dmrs_idx;% LS ChEst just est channel on DMRS REs %12*(startPrb_dmrs + startPrb_est - 2) + 1 : 12*(startPrb_dmrs + startPrb_est + nPrb_est - 2);
...
% save results   
for i = 1 : nl
    H_est(global_est_idx, i) = Y_local2(:,i);
end 

% accumulate R0 and R1;
R0_acc = 0;
R1_acc = 0;
if nl == 1
    num_valid_dmrs = Nf_dmrs;
    Y_local3 = Y_local2;
else % if layers have the same gridIdx, the following is to recover h1 and h2 (removing fOCC); if layers have different gridIdx, the following is to do avg between adj DMRS REs
    num_valid_dmrs = Nf_dmrs/2;
    Y_local3 = 0.5*(Y_local2(1:2:end,:) + Y_local2(2:2:end,:));
end
for i = 1 : nl
    for idx_dmrs = 1:num_valid_dmrs
        R0_acc = R0_acc + conj(Y_local3(idx_dmrs,i))*Y_local3(idx_dmrs,i);
        if idx_dmrs<num_valid_dmrs
            R1_acc = R1_acc + conj(Y_local3(idx_dmrs,i))*Y_local3(idx_dmrs+1,i);
        end
    end
end
