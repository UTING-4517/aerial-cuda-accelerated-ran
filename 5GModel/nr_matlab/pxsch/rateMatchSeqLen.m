...
function [G_harq, G_harq_rvd, G_csi1, G]= rateMatchSeqLen(nBitsHarq,nBitsCsi1,pduBitmap,alphaScaling,...
...
% CSI-PART1 and ULSCH bitstreams, when carried over PUSCH (with or
% without UL-SCH)
...
% dmrsSymLoc_array:     symbol indexes for PUSCH dmrs allocation (Matlab 2 indexing)
...
mScUciSumFroml0 = sum(mScUci(l0:end));                                     % Total number of REs, from symbol l_0 onwards, available for UCI transmission
...
if (~isDataPresent) && (nBitsCsi1 && ~isCsi2Present)                       % Ref. Sec. 6.3.2.1.1 38.212 (This may be redundant if L2 sends nBitsHarq = 2 for this clause)
    if(nBitsHarq <=1)
        nBitsHarq=2;
    end
end
...
if isDataPresent
      G = G_Ulsch -G_harq*(nBitsAckRvd ==0)-G_csi1;
else
    G = 0;
end
