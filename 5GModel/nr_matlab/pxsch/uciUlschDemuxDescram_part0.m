...
function [harq_LLR_descr, csi1_LLR_descr, ulsch_LLR_descr]= ...
    uciUlschDemuxDescram_part0 (harq_LLR_descr, csi1_LLR_descr, ...
    ulsch_LLR_descr, eqOutLLRs, nBitsHarq, nBitsCsi1, ...
    uciOnPuschDeMuxDescr, numDmrsCdmGrpsNoData, isDataPresent)
%%
%INPUT
    ...
    for reIdx = 0 : (nResPerSym - 1)  
        ...
        if(harqReGrids{symIdx + 1}.nRes > 0)
            ...
            if(rvdHarqReFlag)
                ...
                % If assigned, descramble RE LLRs and save to HARQ rateMatch LLR buffer
                if(assignFlag)
                    reLLRs = eqOutLLRs(:,reIdx+1,symIdx+1);
                    if((nBitsHarq == 1) && (qam > 1))
                        reLLRs = descrambleReLLRs_1bit_spx(reLLRs, reIdx, symIdx, c, nl, qam);
                    else
                        reLLRs = descrambleReLLRs_standard(reLLRs, reIdx, symIdx, c);
                    end               
                    rmBufferIdxs = harqReGrids{symIdx + 1}.rmBufferOffset + nBitsPerRe*cumltNumAssignedRes + (0 : (nBitsPerRe - 1));  
                    harq_LLR_descr(rmBufferIdxs + 1) = reLLRs;
                    continue;
                end
            else
        ...
        if((csi1ReGrids{symIdx + 1}.nRes > 0) && (~rvdHarqReFlag))
            ...
            if(assignFlag)
                % If assigned, descramble RE LLRs and save to CSI-P1 rateMatch LLR buffer
                reLLRs = eqOutLLRs(:,reIdx+1,symIdx+1);
                if((nBitsCsi1 == 1) && (qam > 1))
                    reLLRs = descrambleReLLRs_1bit_spx(reLLRs, reIdx, symIdx, c, nl, qam);
                else
                    reLLRs = descrambleReLLRs_standard(reLLRs, reIdx, symIdx, c);
                end               
                rmBufferIdxs = csi1ReGrids{symIdx + 1}.rmBufferOffset + nBitsPerRe*cumltNumAssignedRes + (0 : (nBitsPerRe - 1));  
                csi1_LLR_descr(rmBufferIdxs + 1) = reLLRs;
