...
function hdf5_write_nv(hdf5loc, name, A, varargin)
  ...
  if strcmp('H5T_COMPOUND', H5SrcTypeString)
      ...
      for iName = 1:length(names)
          ...
          elemDims = size(A(1).(names{iName}));
          elemSize = prod(elemDims);
          if elemSize > 1 % not a scalar
              elemTypes{end} = H5T.array_create(elemTypes{end}, elemDims);
          end
      ...
      % ans = 'hello'
      for iName = 1:length(names)
          if prod(size(A(1).(names{iName}))) ==  1 % scalar
              Awrite.(names{iName}) = reshape([A.(names{iName})], size(A));
          else % non-scalar
              Awrite.(names{iName}) = reshape([A.(names{iName})], size(A.(names{iName})));
          end
      end
      %Awrite
