...
function L = Varray_util_chol(A, tri_flag, fp_flag)
    if nargin==1
    ...
    L = Varray(zeros(N,N),fp_flag); % initialize the output matrix
    for j = 1:N
        Lj = L(j,1:j-1); 
%         Lj_value = Lj.value;
        tmp_sum1 = Lj*Lj';%sum(conj(L(j,1:j-1)).*L(j,1:j-1));
        Ljj = sqrt(A(j,j)-tmp_sum1);
        L(j,j) = Ljj;
        Dj_inv = Varray(1.0,fp_flag)/Ljj;
    
%         Lvalue = L.value;
%         tmp_sum2 = Varray(Lvalue(j+1:N,1:j-1)*Lvalue(j,1:j-1)');
        tmp_sum2 = L(j+1:N,1:j-1)*L(j,1:j-1)';
