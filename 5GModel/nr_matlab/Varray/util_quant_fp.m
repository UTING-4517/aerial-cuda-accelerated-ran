...
function y = util_quant_fp(x, E, M)
    
    % reshape input x into a vector
    size_array  = size(x);
    x_is_real   = isreal(x);
    if x_is_real
        x = x(:);
...
%     smallest_normal     = 2^Emin;
    ...
    % reshape y into the original size of x
    if x_is_real
        y = reshape(y, size_array);
    else
        y = reshape(y(1:len_x/2)+1i*y(len_x/2+1:end), size_array);
    end
end