...
function X = Varray_util_forward_sub_3d(L, B, fp_flag)
    if nargin == 2
        fp_flag = 0;        % 0: double format
    end
    [numRows,numEqus,M] = size(B.value);            % M: num of 2d matrices
    X = Varray(zeros(numRows,numEqus,M), fp_flag);
    for idx_row = 1:numRows 
        tmp_sum = pagemtimes(L(idx_row,1:idx_row-1,:), X(1:idx_row-1,:,:));
        X(idx_row,:,:) = pagemtimes((B(idx_row,:,:) - tmp_sum), (Varray(1.0, fp_flag)./L(idx_row,idx_row,:)));
    end 
end
