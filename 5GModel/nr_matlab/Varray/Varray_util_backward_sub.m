...
function X = Varray_util_backward_sub(U, B, fp_flag)
    if nargin == 2
    ...
    for idx_row = numRows:-1:1
        tmp_sum = U(idx_row,idx_row+1:numRows)*X(idx_row+1:numRows,:);
        X(idx_row,:) = (B(idx_row,:) - tmp_sum) * (Varray(1.0, fp_flag)/U(idx_row,idx_row));
    end 
end
