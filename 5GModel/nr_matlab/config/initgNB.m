...
function Mac = gNbMacInitConfig(SysPar)
...
switch configType
    case 'carrier'
        ...
        config.numRxAntSrs          = carrier.Nant_gNB_srs;
...
carrier.Nant_gNB_srs = CarrierConfig.numRxAntSrs;
...
Phy.Config.table.W53 = derive_small_filter_with_vPRBs(5,3,-1,10^(-3),15e3*2^(carrier.mu));

% compute ChEst shift sequences
tau    = (2.0*10^(-6) - 2*10^(-6)/10) / 2; % delay shift

% compute rkhs objects:
Phy.Config.table.push_rkhs_tables = derive_pusch_rkhs_objects;
Phy.Config.table.srs_rkhs_tables  = derive_srs_rkhs_objects;

% compute SRS ChEst filters
Phy.Config.table.W_comb2_nPorts1_wide  = build_SRS_filter(2, 1 , 2.00*10^(-6), 0.4);
Phy.Config.table.W_comb2_nPorts2_wide  = build_SRS_filter(2, 2 , 2.00*10^(-6), 0.4);
Phy.Config.table.W_comb2_nPorts4_wide  = build_SRS_filter(2, 4 , 2.00*10^(-6), 0.4);
Phy.Config.table.W_comb2_nPorts8_wide  = build_SRS_filter(2, 8 , 1.50*10^(-6), 0.4);

Phy.Config.table.W_comb4_nPorts1_wide  = build_SRS_filter(4, 1 , 2.00*10^(-6), 0.4);
Phy.Config.table.W_comb4_nPorts2_wide  = build_SRS_filter(4, 2 , 2.00*10^(-6), 0.4);
Phy.Config.table.W_comb4_nPorts4_wide  = build_SRS_filter(4, 4 , 1.50*10^(-6), 0.4);
Phy.Config.table.W_comb4_nPorts6_wide  = build_SRS_filter(4, 6 , 0.20*10^(-6), 0.4);
Phy.Config.table.W_comb4_nPorts12_wide = build_SRS_filter(4, 12, 0.01*10^(-6), 0.4);
...
[W,noisEstDebias] = build_SRS_filter(2, 8, 0.25*10^(-6), 0);
Phy.Config.table.W_comb2_nPorts8_narrow      = W;
Phy.Config.table.noisEstDebias_comb2_nPorts8 = noisEstDebias;
...
[W,noisEstDebias] = build_SRS_filter(4, 6, 0.01*10^(-6), 0);
Phy.Config.table.W_comb4_nPorts6_narrow      = W;
Phy.Config.table.noisEstDebias_comb4_nPorts6 = noisEstDebias;

[W,noisEstDebias] = build_SRS_filter(4, 12, 0.01*10^(-6), 0);
Phy.Config.table.W_comb4_nPorts12_narrow      = W;
Phy.Config.table.noisEstDebias_comb4_nPorts12 = noisEstDebias;
...
for idxSrsBuff = 0 : (N_SRS_CHEST_BUFF - 1)
    ...
    startValidPrg(idxSrsBuff + 1) = srsChEstBuffPrms.startValidPrg;
    nValidPrg(idxSrsBuff + 1)     = srsChEstBuffPrms.nValidPrg;
end
srsChEstDatabase = [];
srsChEstDatabase.srsChEstBuff = srsChEstBuff;
srsChEstDatabase.startPrbGrps = startPrbGrps;
srsChEstDatabase.startValidPrg = startValidPrg;
srsChEstDatabase.nValidPrg = nValidPrg;
if(N_SRS_CHEST_BUFF > 0)
    srsChEstDatabase.prbGrpSize = SysPar.srsChEstBuff{1}.prbGrpSize;
end
Phy.srsChEstDatabase          = srsChEstDatabase;

Phy.srsState = []; % for mMIMO SRS/BFW/PUSCH BLER simululation
