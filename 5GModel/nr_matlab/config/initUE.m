...
function UE = initUE(SysPar, idxUE)
...
Mac = UeMacInitConfig(SysPar, idxUE);
FAPIconfig = UeMacSendConfigToPhy(Mac);
Phy = UePhyInitConfig(FAPIconfig);
...
function Mac = UeMacInitConfig(SysPar, idxUE)
...
for idxAlloc = 1:length(alloc)
    ...
    switch allocType
        ...
        case 'pusch'
            ...
            pusch{idxPusch}.harqState = struct;
            idxPusch = idxPusch + 1;
        case 'srs'
...
function FAPIconfig = UeMacSendConfigToPhy(Mac)
...
function Phy = UePhyInitConfig(FAPIconfig)
