    ...
    for idx = 1:length(info.Datasets)
        ...
        if (strcmp(dset.Datatype(1).Class, 'H5T_COMPOUND'))     && ...
           ...
           (strcmp(dset.Datatype(1).Type.Member(2).Name, 'im'))
            ...
            %matlab won't make complex integers
            s.(dset.Name) = single(data.re) + (1i * single(data.im));
            %fprintf('%s is complex\n', dset.Name);
        else
            % For non-complex data, just read...
            s.(dset.Name) = h5read(filename, strcat(location, dset.Name)); 
        end
    end
end
