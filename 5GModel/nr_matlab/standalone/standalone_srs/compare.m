...
function  [metrics] = compare(x_ref, x_meas, metric_name, plot_fig, pruneZeros, ref_src_name, meas_src_name)
    ...
    if min(x_ref) < eps
        idxs_x_ref = find(x_ref < eps);
    else
        idxs_x_ref = [];
    end
    if min(x_meas) < eps
        idxs_x_meas = find(x_meas < eps);
    else
        idxs_x_meas = [];
    end
    ...
    metrics.avg_rel_err_pwr_db = 10*log10(mean(metrics.err_pwr)./mean(metrics.sig_pwr));
    ...
    if plot_fig
        if(~isreal(x_ref))
            ...
            subplot(2,1,2);
            plot(x_ref, 'og'); hold on; plot(real(x_meas), '+b');                        
