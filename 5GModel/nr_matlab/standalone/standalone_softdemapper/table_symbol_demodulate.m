...
function out = table_symbol_demodulate(in, mod, QAM_noise_var, varargin)
  ...
  % Specify additional arguments 'Precision', 'fp16' to convert the
  % table values to fp16 before interpolating.
  % Example:
  % llr = table_symbol_demodulate(syms, 'QAM256', 1, 'Precision', 'fp16');
  %---------------------------------------------------------------------
  ...
  use_fp16 = false;
  if nargin == 5
      if all(strcmp(varargin, {'Precision', 'fp16'}))
          use_fp16 = true;
      else
  ...
  if use_fp16
      for colidx = 1:length(T.Properties.VariableNames)
          colname = T.Properties.VariableNames{colidx};
          if startsWith(colname, 'bit')
              % Truncate to an fp16 representation
              colfp16 = fp16nv(T.(colname), 2);
              % Convert back to fp16 for interpolation below
              T.(colname) = double(colfp16);
          end
      end
  end
