...
int main(int argc, char* argv[]) {
    ...
    int num_TBs = 1;
    auto input_data = typed_tensor_from_dataset<CUPHY_R_8U, pinned_alloc>(input_file.open_dataset("tb0_layer_mapped"));
    auto output_data = typed_tensor_from_dataset<CUPHY_C_64F, pinned_alloc>(input_file.open_dataset("tb0_qams"));
    ...
    for (int element_start = 0; element_start < rate_matched_bits; element_start += ELEMENT_SIZE)  {
            ...
            for (int offset = 0; ((offset < ELEMENT_SIZE) && (element_start + offset < rate_matched_bits)); offset++) {
                uint32_t bit = input_data({element_start + offset});
