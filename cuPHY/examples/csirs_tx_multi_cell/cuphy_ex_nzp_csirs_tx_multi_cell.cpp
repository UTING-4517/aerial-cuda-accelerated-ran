...
int main(int argc, char* argv[])
{
                 ...
                 switch(argv[iArg][2])
                 {
                    case 'G':
                        ...
                        if(((++iArg >= argc) || (1 != sscanf(argv[iArg], "%i", &SMsPerGreenCtx))) || (SMsPerGreenCtx == 0))
                        {
                            ...
                            NVLOGE_FMT(NVLOG_CSIRS, AERIAL_CUPHY_EVENT,  "ERROR: Invalid or missing useGreenCtxs argument (--G)");
                            exit(1);
                        }
                        ++iArg;
                        break;
    ...
    if(useGreenCtxs)
    {
        ...
        NVLOGC_FMT(NVLOG_CSIRS, "CSI-RS green context will have access to {} SMs ({} SMs requested).", csirs_green_ctx.getSmCount(), SMsPerGreenCtx);
    }
#endif
    ...
    for(int idxSlot = 0; idxSlot < num_slots; idxSlot++)
    {
        ...
        NVLOGC_FMT(NVLOG_CSIRS, "Slot # {}, CSIRS pipeline(s) CPU Time: {:.2f} us total (summed over {} CSIRS objects, avg. over {} iterations)",
                   idxSlot, avgElapsedTimesUs[ELAPSED_CPU_RUN] + avgElapsedTimesUs[ELAPSED_CPU_SETUP], num_csirs_objects, num_iterations);

        NVLOGC_FMT(NVLOG_CSIRS, "--> Slot # {}: CPU-Run {:.2f} us (min {:.2f} us, max {:.2f} us), CPU-Setup {:.2f} us (min {:.2f} us, max {:.2f} us)", idxSlot,
