...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if(txtInputFlag)
        {
            ...
            size_t lastPathSepIndex = textFilename.find_last_of(pathSeparator); // pathSeparator will be "\" on windows, and "/" otherwise
            std::string textFileFolder=textFilename.substr(0, lastPathSepIndex + 1); // text file folder name
            ...
            if ( perfCurveRequestFile.is_open() ) {
                while ( perfCurveRequestFile ) {
                    ...
                    if(!currentLine.empty())
                    {
                        ...
                        std::string folderName = textFileFolder + currentLine.substr(firstEqLoc + 1, firstComLoc - firstEqLoc - 1);
