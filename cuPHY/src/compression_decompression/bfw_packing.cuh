    ...
    const uint8_t& compbits)  // Number of compressed bits. 16 means no compression
{
    ...
    int16_t beam_id = (compParam >> 8);
    offset += (beam_id >= 0) * 2;

    // Compression parameter. Don't write if no compression is used.
    if(tx_prb == 0 && compbits < 16){
        //uint32_t* bundle_header = reinterpret_cast<uint32_t*>(sm);
        //bundle_header[0] = compParam;
        sm[0] = compParam;
        sm[2] = compParam >> 8;
        sm[1] = compParam >> 16;
    }
...
__device__ inline void unpackInput(
    ...
    const bool packed_beam_id,         // Whether the beam_id is packed in the compressed data
    const uint8_t compbits)            // Number of compressed bits
{
    if(compbits < 16)
    {
        // Get the shift value from the first byte
        compParam = input[0];
        
        // Calculate base offset for data - always skip the shift byte
        int offset = tx_prb * compbits + packed_beam_id * 2 + 1;
