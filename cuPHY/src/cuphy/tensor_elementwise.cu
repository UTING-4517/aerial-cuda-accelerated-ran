...
void update_descriptors_for_broadcast(tensor_desc& tSrcA,
                                      tensor_desc& tSrcB)
{
    cuphy::vec<int, CUPHY_DIM_MAX> Adim     = tSrcA.layout().dimensions;
    cuphy::vec<int, CUPHY_DIM_MAX> Bdim     = tSrcB.layout().dimensions;
    cuphy::vec<int, CUPHY_DIM_MAX> Astrides = tSrcA.layout().strides;
    cuphy::vec<int, CUPHY_DIM_MAX> Bstrides = tSrcB.layout().strides;
...
{
    const cuphy::vec<int, CUPHY_DIM_MAX>& dimDst = tDst.layout().dimensions;
    const cuphy::vec<int, CUPHY_DIM_MAX>& dimA   = tSrcA.layout().dimensions;
    const cuphy::vec<int, CUPHY_DIM_MAX>& dimB   = tSrcB.layout().dimensions;
    for(int i = 0; i < CUPHY_DIM_MAX; ++i)
    {
        if(dimA[i] != dimB[i])
        {
