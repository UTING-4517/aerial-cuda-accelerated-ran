...
template <typename T>
__global__ void rm_decoder_3(const __grid_constant__ rmDecoderDynDescr_t desc)
{
    const uint32_t * __restrict__ codebook = desc.pRmCodebook;
    uint16_t  numCW = desc.nCws;
    cuphyRmCwPrm_t*  pCwPrmsGpu = desc.pCwPrmsGpu;
                            ...
                            cudaStream_t               strm)                        // stream to perform copy
{
    ...
    pLaunchCfg->kernelArgs[0]                       = pCpuDynDesc;
