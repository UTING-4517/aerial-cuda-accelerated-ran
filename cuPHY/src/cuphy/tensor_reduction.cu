...
cuphy::vec<int, CUPHY_DIM_MAX> collapse_dim(const cuphy::vec<int, CUPHY_DIM_MAX>& src, int dim)
{
    cuphy::vec<int, CUPHY_DIM_MAX> dst = src;
    dst[dim] = 1;
    return dst;
}
                                          ...
                                          cuphyReductionOp_t redOp)
{
    ...
    // expect.
    cuphy::vec<int, CUPHY_DIM_MAX> rdims = collapse_dim(tSrc.layout().dimensions, dim);
                               ...
                               cudaStream_t       strm)
{
    ...
    case CUPHY_REDUCTION_SUM:
        s = tensor_reduction_sum(tDstLayout,
                                 ...
                                 strm);
        break;
    /* Using CUPHY_REDUCTION_MIN and CUPHY_REDUCTION_MAX are aborted during call to 
    * validate_tensor_descriptors() as they are currently unimplemented so 
    * these cases should never get reached
    * Additionally, we exclude the error case from coverage analysis
    */
    /*VCAST_DONT_INSTRUMENT_START*/
