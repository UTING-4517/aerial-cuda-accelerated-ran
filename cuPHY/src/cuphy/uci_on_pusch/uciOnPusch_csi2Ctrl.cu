...
static __device__ __constant__ float    d_betaOffsetCsiMapping[19]    = {1.125, 1.250, 1.375, 1.625, 1.750, 2.000, 2.250, 2.500, 2.875, 3.125, 3.500, 4.000, 5.000, 6.250, 8.000, 10.000, 12.625, 15.875, 20.000};
 ...
 __global__ void uciOnPuschCsi2CtrlKernel(uciOnPuschCsi2CtrlDynDescr_t* pDesc)
 {
    ...
    uint16_t                 nCsi2Reports       = perTbPrms.nCsi2Reports;
    cuphyCalcCsi2SizePrm_t*  pCalcCsi2SizePrms  = perTbPrms.calcCsi2SizePrms;

    // FAPIv3 method for computing num CSI2 bits:
    uint8_t             enableCsiP2Fapiv3 = pDesc->enableCsiP2Fapiv3;
    cuphyCsi2MapPrm_t*  pCsi2MapPrmsGpu   = cellStatPrms.pCsi2MapPrm;
    uint16_t*           pCsi2MapBufferGpu = cellStatPrms.pCsi2MapBuffer;
    ...
    uint8_t*                 pDecodedCsiPart1   = pDesc->pUciPayloads + pDesc->csi2ToBuffersMap[csi2Idx].csi1PayloadByteOffset;
    if(enableCsiP2Fapiv3 == 0)
    {
        pDecodedCsiPart1 += rankOffsetMultiple;
    }
    ...
    }else if((enableCsiP2Fapiv3 == 1))
    {
        uint16_t nCsi2Bits = 0;
        for(int csi2ReportIdx = 0; csi2ReportIdx < nCsi2Reports; csi2ReportIdx++)
        {
            // paramaters used to compute bitlength of this csi2 report:
            cuphyCalcCsi2SizePrm_t& calcCsi2SizePrm = pCalcCsi2SizePrms[csi2ReportIdx];

            // compute bitlength of aggregated csi1 paramaters:
            uint16_t                aggrCsi1PrmsBitLength = 0;
            for(int csi1PrmIdx = 0; csi1PrmIdx < calcCsi2SizePrm.nPart1Prms; ++csi1PrmIdx)
            {
                aggrCsi1PrmsBitLength += calcCsi2SizePrm.prmSizes[csi1PrmIdx];
            }        

           // aggregate csi1 paramaters:
           uint16_t bitIdxWithinAggrCsiPrm = aggrCsi1PrmsBitLength - 1;
           uint16_t aggrCsi1Prms           = 0;
            for(int csi1PrmIdx = 0; csi1PrmIdx < calcCsi2SizePrm.nPart1Prms; ++csi1PrmIdx)
            {
                // location and size of csi1 paramater:
                uint8_t  prmSize   = calcCsi2SizePrm.prmSizes[csi1PrmIdx];
                uint16_t prmOffset = calcCsi2SizePrm.prmOffsets[csi1PrmIdx];

                for(int csi1PrmBitIdx = 0; csi1PrmBitIdx < prmSize; ++csi1PrmBitIdx)
                {
                    // extract csi1Prm bit from csi1 payload: 
                    uint16_t bitIdx           = csi1PrmBitIdx + prmOffset;
                    uint16_t byteIdx          = bitIdx / 8;
                    uint16_t bitIdxWithinByte = bitIdx - byteIdx * 8;
                    uint16_t csi1PayloadBit   = static_cast<uint16_t>((pDecodedCsiPart1[byteIdx] >> bitIdxWithinByte) & (0x1));

                    // aggreate the csi1Prm bit:
                    aggrCsi1Prms = (csi1PayloadBit << bitIdxWithinAggrCsiPrm) | aggrCsi1Prms;

                    // update aggreCsi1Prm bit location:
                    bitIdxWithinAggrCsiPrm = bitIdxWithinAggrCsiPrm - 1;
                }
            }

            // use the aggregated CSI-P1 prms to lookup CSI-P2 size:
            uint16_t csi2sizeMapIdx   =  calcCsi2SizePrm.csi2sizeMapIdx;
            uint32_t csi2MapStartIdx  =  pCsi2MapPrmsGpu[csi2sizeMapIdx].csi2MapStartIdx;
            nCsi2Bits                +=  pCsi2MapBufferGpu[csi2MapStartIdx + aggrCsi1Prms];

     //       printf("\n\n nCsi2Bits = %d \n\n", nCsi2Bits);
        }
        *pNumCsi2Bits = nCsi2Bits;
    }else
                                ...
                                cudaStream_t                         strm)
{
    ...
    pCpuDynDesc->enableCsiP2Fapiv3 = enableCsiP2Fapiv3;
