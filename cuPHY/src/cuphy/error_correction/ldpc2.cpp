...
int32_t get_device_max_shmem_per_block_optin()
{
    ...
    if(shmem_value <= 0)
    {
        int sharedMemPerBlockOptIn = 0;
        e = cudaDeviceGetAttribute(&sharedMemPerBlockOptIn, cudaDevAttrMaxSharedMemoryPerBlockOptin, deviceIndex);
        if(cudaSuccess != e)
        {
            return -1;
        }
        shmem_value = static_cast<int32_t>(sharedMemPerBlockOptIn);
