...
public:
     ...
     * @param[in] cuStream: CUDA stream for synchronization
     */
    void writeDbgBufSynch(cudaStream_t cuStream);

    // debug functions:
    template <fmtlog::LogLevel log_level=fmtlog::DBG>
    static void printStaticApiPrms(cuphySsbStatPrms_t const* pStaticPrms);
    template <fmtlog::LogLevel log_level=fmtlog::DBG>
    static void printDynApiPrms(cuphySsbDynPrms_t const* pDynPrms);
...
private:
    ...
    // Static counter for unique H5 dataset naming across all SSB objects and write operations
    static std::atomic<int> s_ssbIdCounter;
