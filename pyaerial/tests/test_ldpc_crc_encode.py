...
from aerial.phy5g.ldpc.crc_encode import CrcEncoder
...
def test_ldpc_crc_encoder(cuda_stream, test_case_number, cupy):
    """Test CrcEncoder."""
    ...
    tb_input_array = np.array(input_file["InputData"])
    tb_sizes = np.array(input_file["cw_pars"]["tbSize"])

    tb_inputs = []
    idx = 0
    for tb_size in tb_sizes:
        tb_inputs.append(tb_input_array[idx:idx + tb_size])
        idx += tb_size
    tb_sizes = [tb_size * 8 for tb_size in tb_sizes]
    ...
    crc_out = crc_encoder.encode(tb_inputs=tb_inputs, tb_sizes=tb_sizes, code_rates=code_rates)

    if cupy:
        crc_out = [crc.get() for crc in crc_out]

    for ue_idx in range(num_ues):
        ref_output = np.array(input_file[f"tb{ue_idx}_cbs"]).astype(np.float32).T
        assert np.array_equal(crc_out[ue_idx], ref_output)
