...
def test_ldpc_decoder(pusch_config, cuda_stream, test_case_number, h2d):
    """Test LDPC decoder."""
    ...
    if h2d:
        input_llrs = [cp.array(input_llr, order='F', dtype=cp.float16) for input_llr in input_llrs]
    ...
    if h2d:
        decoded_bits = [elem.get(order='F') for elem in decoded_bits]
...
def test_ldpc_decoder_soft_output(cuda_stream, test_case_number, h2d):
    ...
    if h2d:
        ldpc_decoder_input_llr = cp.array(ldpc_decoder_input_llr, order='F', dtype=cp.float16)
    ...
    soft_bits = ldpc_decoder.get_soft_bits()[0]
    if h2d:
        soft_bits = soft_bits.get(order='F')
