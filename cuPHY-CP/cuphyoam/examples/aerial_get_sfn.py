...
def GetSFN(channel):
    ...
    response = stub.GetSFN(aerial_common_pb2.GenericRequest(name=''))
    return response.sfn, response.slot
...
if __name__ == '__main__':
    ...
    with grpc.insecure_channel('localhost:50051') as channel:
        while 1:
            sfn, slot = GetSFN(channel)
            logging.info(f"Current SFN: {sfn} Slot: {slot}")
            time.sleep(1)
